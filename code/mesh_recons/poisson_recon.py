...
if FLAGS.nbody: fpath = cscratch + "nbody_%d_nx%d_ny%d_poisson_p%0.2f%s/"%(nc, FLAGS.nx, FLAGS.ny, FLAGS.plambda, FLAGS.suffix)
else: fpath = cscratch + "lpt_%d_nx%d_ny%d_poisson_p%0.2f%s/"%(nc, FLAGS.nx, FLAGS.ny, FLAGS.plambda, FLAGS.suffix) # 
print(fpath)
...
def recon_model(mesh, data, R0, x0, nc=FLAGS.nc, bs=FLAGS.box_size, batch_size=FLAGS.batch_size,
    ...
    chisq = mtf.cwise(_cwise_logprob, [final_fieldsm, mtfdata], output_dtype=tf.float32) # 
...
##    #
    ...
    sample = mtf.cwise(_cwise_sample, [final_fieldsm, mtfdata], output_dtype=tf.float32) # 
...
##    
    ...
    plambda = FLAGS.plambda
    ic, fin = np.load('../data/poisson_N%03d/ic.npy'%nc), np.load('../data/poisson_N%03d/psample_%0.2f.npy'%(nc, plambda))
    print('Data loaded')
