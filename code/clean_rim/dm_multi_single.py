...
from modeldm import DMData, check_2pt, check_im, get_data
...
if args.parallel: suffpath = '_dm_w%d'%world_size + args.suffix
...
minic, minfin = datamodel.reconstruct(tf.constant(y_test), 
                                      RRs=[1.0, 0.5,  0.0], niter=args.rim_iter*20, lr=0.1, x_init=x_init, useprior=True)
pred_adam, _ = datamodel.reconstruct(tf.constant(y_test), 
                                      RRs=[0.0], niter=args.rim_iter, lr=0.1, x_init=x_init, useprior=True)
pred_adam10, _ = datamodel.reconstruct(tf.constant(y_test), 
                                      RRs=[0.0], niter=args.rim_iter*10, lr=0.1, x_init=x_init, useprior=True)
