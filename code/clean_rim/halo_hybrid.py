...
from rim_utils1d import  RIMHybrid, build_rim1d
...
suffpath = '_hybrid_cutoff_w%d'%world_size + args.suffix
...
with strategy.scope():
    rim =  RIMHybrid(64, 8, args.rim_iter, kbinmap, args, droprate=0.05, regrate=1e-5)
...
def train_step(inputs):
    ...
    if len(rim.trainable_variables) == 0:
        ...
        print("##Hack to get trainiable variables")
        j = 0
        b, c = y[j:j+1],  x_true[j:j+1]
        if args.stdinit:
            a = b[:, 1] / b1eul 
        ...
        print("#number of trainable variables : ", len(rim.trainable_variables))
    #
    gradients = [0.]*len(rim.trainable_variables)
    for j in range(args.batch_size // world_size):
        ...
        for l in range(len(grads)):
            gradients[l] = gradients[l] + grads[l] 
