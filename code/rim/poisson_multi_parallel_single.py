...
from rim_utils import build_rim, build_rim_parallel_single, myAdam
import flowpm
...
parser.add_argument('--sims_in_loop', type=int, default=2, help='Number of sims in a loop')
parser.add_argument('--parallel', type=str2bool, default=True, help='Parallel or Split')


args = parser.parse_args()
if args.parallel : pass
else:
...
def test_step(inputs):
    x_true, y = inputs
    x_init = tf.random.normal(x_true.shape)
    x_pred, _ = rim(x_init, y, grad_fn, x_true)
...
if args.parallel: suffpath = '_p%03d_parallel_single'%(plambda*100) + args.suffix
else: suffpath = '_p%03d_split_single'%(plambda*100) + args.suffix
...
for epoch in range(EPOCHS):
    ...
    for x in train_dist_dataset:
        print("from train_dist_dataset ", len(x), x[0].values[0].shape)
        startb = time.time()
    ...
    for x in test_dist_dataset:
        print('Testing')
        print("from test_dist_dataset ", len(x), x[0].values[0].shape)
        a, b, c, d = distributed_test_step(x)
        print(a.values[0].shape, b.values[0].shape, c.values[0].shape, d.values[0].shape)
        pred, x_init, xx, yy = a.values[0], b.values[0], c.values[0], d.values[0]
        pred = pred
